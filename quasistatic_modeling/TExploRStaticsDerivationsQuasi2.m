...
% TExploRStaticsDerivationsQuasi2(0, 90);

classdef TExploRStaticsDerivationsQuasi2 < handle
...
%             zB = zB/norm(zB);
    ...
    methods
        function obj = TExploRStaticsDerivationsQuasi2(theta1_deg, theta2_deg) % Only need theta1, theta2 inputs
...
%             2 - Other robot parameters
        ...
        function staticPoses = computeStaticPoses(obj)
            ...
            %figure(1)
            fig = figure(1)
            set(fig, 'Position', [1, 1, 1800, 600]);  % [left, bottom, width, height] in pixels
                for ii = 1:1:4    
                    ...
                    obj.increment = ii;
                     if ii == 1
                        ...
                        %for h = 171:-10:1
                            ...
                            obj.quasiStatic2(obj.staticPoses.Case(ii).Tsb,obj.theta1,obj.theta2,obj.staticPoses.Case(ii).phi1,obj.staticPoses.Case(ii).phi2, obj.increment, obj.iter);
                            obj.iter = obj.iter + 1;
                        end
        ...
        function quasiStatic2(obj, Tsb, theta1, theta2, phi1, phi2, ii, iter)
            % Pre-transformation
